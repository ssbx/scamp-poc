...
static int ang2pix_nest_z_phi(long nside_, double z, double phi) {
    ...
    } else /* polar region, za > 2/3 */
    {
        ...
        if (jp >= nside_)
            jp = nside_ - 1; /* for points too close to the boundary */
        if (jm >= nside_)
            jm = nside_ - 1;
        if (z >= 0) {
            face_num = ntt; /* in {0,3} */
...
static void pix2ang_ring_z_phi64(int64_t nside_, int64_t pix, double *z,
        double *s, double *phi) {
...
void vec2pix_ring64(int64_t nside, const double *vec, int64_t *ipix) {
    ...
    double sth =
            (fabs(cth) > 0.99) ?
                    sqrt(vec[0] * vec[0] + vec[1] * vec[1]) / vlen : -5;
